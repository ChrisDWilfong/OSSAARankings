Public Class LoginBAOSheree
    ...
    Protected Sub cmdLogin_Click(ByVal sender As Object, ByVal e As EventArgs) Handles cmdLogin.Click
        ...
        strInvalidCodeMessage = "Invalid code.  You are not identified as qualifying to enter Baseball Playoff Availability in our system (OSSAA enrolled official and passed Part 1).  Please email Sheree Riddell (sriddell@ossaa.com) or call 405.840.1116"
        ...
        Else
            ...
            'Dim strSQL As String = "SELECT TOP 1 id FROM prodOfficials WHERE strEmail = '" & Me.txtUsername.Text & "' And ([ysnCurrentBasketball] = -1) And (intTestBasketball >= 75) And (intOSSAAID > 0)"
            Dim strSQL As String
            If InStr(Me.txtUsername.Text, "@") > 0 Then
                strSQL = "SELECT TOP 1 id FROM viewOfficialsBaseballEligiblePlayoffs WHERE strEmail = '" & Me.txtUsername.Text & "'"
            Else
                strSQL = "SELECT TOP 1 id FROM viewOfficialsBaseballEligiblePlayoffs WHERE intOSSAAID = '" & Me.txtUsername.Text & "'"
            End If
