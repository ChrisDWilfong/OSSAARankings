Public Class sbSoccer
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not IsPostBack Then
            ...
            Select Case strSportGenderClass
                Case "SoccerBoys6A"
                    strHeader = "CLASS 6A SOCCER (BOYS) "
                Case "SoccerGirls6A"
                    strHeader = "CLASS 6A SOCCER (GIRLS) "
                Case "SoccerBoys5A"
                    strHeader = "CLASS 5A SOCCER (BOYS) "
                Case "SoccerGirls5A"
                    strHeader = "CLASS 5A SOCCER (GIRLS) "
                Case "SoccerBoys4A"
                    strHeader = "CLASS 4A SOCCER (BOYS) "
                Case "SoccerGirls4A"
                    strHeader = "CLASS 4A SOCCER (GIRLS) "
                Case Else
                    strHeader = "CLASS 6A SOCCER (BOYS) "
                    strSportGenderClass = "SoccerBoys6A"
            End Select
    ...
    Public Sub LoadGames(strSportGenderClass As String, strLevel As String, strGameDate As String)
        ...
        If strLevel = "" Then
            If strGameDate = "" Then
                strSQL = strSQL & "WHERE tblSchedules.sportID = '" & strSportGenderClass & "' AND tblSchedules.intYear = " & clsFunctions.GetCurrentYear & " "
            Else
                strSQL = strSQL & "WHERE tblSchedules.sportID = '" & strSportGenderClass & "' AND tblSchedules.intYear = " & clsFunctions.GetCurrentYear & " AND gameDate = '" & strGameDate & "' "
            End If
        Else
            If strGameDate = "" Then
