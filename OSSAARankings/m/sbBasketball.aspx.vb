Public Class sbBasketball
        ...
        If Not IsPostBack Then
            ...
            Select Case strSportGenderClass
                Case "BasketballGirls6A"
                    strHeader = "CLASS 6A GIRLS "
                Case "BasketballGirls5A"
                    strHeader = "CLASS 5A GIRLS "
                Case "BasketballGirls4A"
                    strHeader = "CLASS 4A GIRLS "
                Case "BasketballGirls3A"
                    strHeader = "CLASS 3A GIRLS "
                Case "BasketballGirls2A"
                    strHeader = "CLASS 2A GIRLS "
                Case "BasketballGirlsA"
                    strHeader = "CLASS A GIRLS "
                Case "BasketballGirlsB"
                    strHeader = "CLASS B GIRLS "
                Case "BasketballBoys6A"
                    strHeader = "CLASS 6A BOYS "
                Case "BasketballBoys5A"
                    strHeader = "CLASS 5A BOYS "
                Case "BasketballBoys4A"
                    strHeader = "CLASS 4A BOYS "
                Case "BasketballBoys3A"
                    strHeader = "CLASS 3A BOYS "
                Case "BasketballBoys2A"
                    strHeader = "CLASS 2A BOYS "
                Case "BasketballBoysA"
                    strHeader = "CLASS A BOYS "
                Case "BasketballBoysB"
                    strHeader = "CLASS B BOYS "
                Case Else
                    strHeader = ""
            End Select
    ...
    Public Sub LoadGames(strSportGenderClass As String, strLevel As String, strGameDate As String)
        ...
        'Else
        '    strSQL = strSQL & "WHERE tblSchedules.sportID = '" & strSportGenderClass & "' AND tblSchedules.strType = '" & strLevel & "' AND tblSchedules.intYear = " & clsFunctions.GetCurrentYear & " AND gameDate = '" & strGameDate & "' "
        'End If
        'strSQL = strSQL & "ORDER BY allCoachesDetail.SchoolName"
        ...
        For x = 0 To ds.Tables(0).Rows.Count - 1
            With ds.Tables(0).Rows(x)
                ...
                objControl.Text = GetScore(.Item("SchoolNameTop"), .Item("SchoolNameBottom"), .Item("TotalScore"), .Item("oTotalScore"), .Item("rank"), .Item("orank"), strStatus)
    ...
    Public Function GetScore(strTopTeam As String, strBottomTeam As String, intTopScore As Integer, intBottomScore As Integer, rank As Integer, orank As Integer, strStatus As String) As String
        Dim strReturn As String

        strReturn = "<table runat='server' width='90%' height='120px'>"
        strReturn = strReturn & "<tr height='50%'>"
        strReturn = strReturn & "<td width='65%'>"
        If (strStatus = "F" Or strStatus = "OT") And intTopScore > intBottomScore Then
