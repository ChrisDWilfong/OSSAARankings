...
Partial Class ucViewScheduleFPRegionals
    ...
    Shared intRegionals As Integer = 8
    Shared gSportGenderKey As String = "FPSoftball"
    Shared gSportDisplay As String = "Fast-Pitch"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            Session("scoreboardscrollerClass") = InitializeDataSet(Session("gScoreboardClass"), intRegionals)
    ...
    Public Function InitializeDataSet(ByVal strSportID As String, ByVal intRows As Integer) As String
        ...
        strSQL = "SELECT teamID, schoolID, sportID, playoffsRegional, ysnHostRegional, SchoolName "
        strSQL = strSQL & "FROM dbo.tblTeams "
        strSQL = strSQL & "WHERE (sportID = '" & gSportGenderKey & strSportID & "') AND (playoffsRegional > 0) "
        strSQL = strSQL & "ORDER BY playoffsRegional"

        dsHeader = SqlHelper.ExecuteDataset(SqlHelper.SQLConnection, CommandType.Text, strSQL)

        strSQL = "SELECT tblTeams.SchoolName, tblTeams_1.SchoolName AS oSchoolName, dbo.tblTeams.playoffsRegional, "
        strSQL = strSQL & "dbo.tblTeams.ysnHostRegional, dbo.tblSchedules.teamID, dbo.tblSchedules.oTeamID, dbo.tblSchedules.sportID, "
        strSQL = strSQL & "dbo.tblSports.strPlayoffsScheduleShowRegionals, dbo.tblSchedules.gamedate, dbo.tblSchedules.gametime, "
        ...
        strSQL = strSQL & "WHERE ((dbo.tblTeams.sportID = '" & gSportGenderKey & strSportID & "') AND (dbo.tblTeams.playoffsRegional > 0)) "
        strSQL = strSQL & "AND (dbo.tblSchedules.strType = 'TOSSAAR') AND (dbo.tblSchedules.ysnActive > 0) "
        strSQL = strSQL & "ORDER BY dbo.tblTeams.playoffsRegional, tblSchedules.gameDate, tblSchedules.gameTime, tblTeams.ysnHostRegional"
        ...
        For x = 0 To intRows - 1
            ...
            For y = 0 To dsHeader.Tables(0).Rows.Count - 1
                If dsHeader.Tables(0).Rows(y).Item("playoffsRegional") = (x + 1) Then
                    If dsHeader.Tables(0).Rows(y).Item("ysnHostRegional") <> 0 Then
                        'strHeader = strSpanBold & "District #" & (x + 1) & " @ " & dsHeader.Tables(0).Rows(y).Item("SchoolName") & "</span><br>"
                        strHeader = strSpanBold & "Regional @ " & dsHeader.Tables(0).Rows(y).Item("SchoolName") & "</span><br>"
                    End If
                End If
            Next
            If strHeader = "" Then
                strHeader = strSpanBold & "Regional #" & (x + 1) & "</span><br>"
            End If

            For y = 0 To ds.Tables(0).Rows.Count - 1
                If ds.Tables(0).Rows(y).Item("playoffsRegional") = (x + 1) Then
    ...
    Private Sub cboClass_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboClass.SelectedIndexChanged
        If rowHeader.Visible Then
            Session("gScoreboardClass") = cboClass.SelectedValue
        End If
        'lblTable.Text = InitializeDataSet(32)
        Session("content") = InitializeDataSet(cboClass.SelectedValue, intRegionals)
        RefreshHeader()
    End Sub

    Public Sub RefreshHeader()
        Dim strSport As String = ""
        strSport = Session("gSportDisplay") & " Regionals Schedules "
